...
x0100 ; no keyboard      ; Set when user isn't using keyboard movement flags, and instead, magnitude + direction
x0200 ; radial movement  ; Set when magnitude and direction is used to determine movement
...
Format:
> `01 flags(input flags) vf(world mouse x) vf(world mouse y) no keys?[vf(gamepad x axis) vf(gamepad y axis)]`
